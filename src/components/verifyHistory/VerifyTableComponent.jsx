...
import { Download, FolderSearch, RotateCw, X, ZoomIn, ZoomOut } from "lucide-react";
...
const VerifyTableComponent = ({
  ...
  const openPreview = useCallback(
    async (submissionId, fileName) => {
      ...
      try {
        ...
        if (!response) throw new Error("No URL returned");
        ...
        console.error("Failed to load Tech Log:", error);
  ...
  const filteredDataForTable = useMemo(() => {
    ...
    if (data?.AdminFlights && Array.isArray(data.AdminFlights)) return data.AdminFlights;
    return actualData;
  }, [data]);

  const flatData = useMemo(() => {
    if (!filteredDataForTable || filteredDataForTable.length === 0) return [];

    return filteredDataForTable.flatMap((log, logIndex) => {
      const entries = log.entries ? log.entries : [log];
      return entries
        .filter((_, entryIndex) => showPilotData || entryIndex === 1) // same logic as TableComponent
  ...
  const meta = useMemo(
    ...
    [openPreview, isLoadingPreview, getFileType]
  );

  // Helper to render group-level cells (only on first row of group)
  const getCenterCell = ({ row, getValue }) => {
    ...
    return (
      <td
        ...
        className="border border-gray-400 text-xs text-center"
      >
        {value}
      </td>
    );
  ...
  const centerColumns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <td
            rowSpan={row.original._rowSpan}
            className="border border-gray-400 text-center font-medium"
          >
    ...
    },
    {
      ...
      cell: ({ row, table }) => {
        ...
        const isLoadingThis = loadingSubmissionsRef.current.has(submissionId);
        const fileType = table.options.meta.getFileType(TechLogURL);
        const Icon = fileType === "pdf" ? FaFilePdf : FaImage;
        const iconColor = fileType === "pdf" ? "text-red-600" : "text-primary";
        ...
        return (
          ...
          >
            <button
              onClick={() => {
                if (!hasValidUrl || isLoadingThis) return;
              ...
              disabled={!hasValidUrl || isLoadingThis}
              className={`p-1.5 rounded transition-colors ${hoverColor} ${hasValidUrl && !isLoadingThis
  ...
  const renderCellsWithKeys = (cells) =>
    cells
      .map((cell, i) => {
        ...
        return rendered ? (
          <React.Fragment
            key={cell.column.id || cell.column.accessorKey || i}
          >
            {rendered}
          </React.Fragment>
        ) : null;
                  ...
                  return (
                    <tr
                      ...
                      className={`h-8 border-b border-gray-400 transition-colors 
                        ${row.original._entryIndex === 0 ? "hover:bg-gray-50" : ""}`}
                    >
                      {renderCellsWithKeys(
...
export default VerifyTableComponent;